# ZcChat2 for Android

一个模仿 Galgame 演出效果的AI桌宠 —— [ZcChat2](https://github.com/Zao-chen/ZcChat2) 的移动端版本

<img width="1376" height="768" alt="ZcChat2" src="https://github.com/user-attachments/assets/6c1941e7-06a2-4df0-bee5-71a348085eb7" />

## 🎯项目介绍

为竖屏设备设计的版本，和 Galgame 角色在手机上对话！


## 🚀 快速入门

使用方式与 [ZcChat2](https://github.com/Zao-chen/ZcChat2?tab=readme-ov-file#-%E5%BF%AB%E9%80%9F%E5%85%A5%E9%97%A8) 保持一致，角色与插件格式完全兼容，可直接上手使用。

## 🤗 如何贡献

ZcChat2ForMobile是一个开源项目。参与ZcChat2项目的方法有很多！

* **为项目做出贡献** ：有兴趣做出贡献吗？欢迎[Pull Request](https://github.com/Zao-chen/ZcChat2ForMobile/pulls)！
* **报告BUG、建议** ：有BUG或有功能请求？请通过[Issues](https://github.com/Zao-chen/ZcChat2ForMobile/issues)提交它们。
* **或者……** ：给项目来一个star⭐怎么样？

## 🔗 相关链接

- [Zao-chen/ZcChat2: 一个模仿 Galgame 演出效果的AI桌宠2.0](https://github.com/Zao-chen/ZcChat2)
- [Zao-chen/ZcChat: 一个模仿Galgame效果的AI桌宠 | An AI desktop pet that mimics the effects of a Galgame](https://github.com/Zao-chen/ZcChat)
- [Flutter - Build apps for any screen](https://flutter.dev/)

注：本项目在开发过程中广泛使用了 AI 辅助。
